<script>
  import * as utils from './utils/loadModel';
  ...
  const loadAssests = async (url) => {
    const alienMan = await utils.loadModels(url);
    return alienMan
  }

  loadAssests('/assets/models/alien/alienSuit.gltf')
 ...
 // import {OrbitControls} from "three/examples/jsm/controls/OrbitControls";
  ...
  $: if (poses && mesh) {
    ...
    let x1 = -((leftShoulder.position.x / VIDEO_WIDTH) * 2 - 1);
    let x2 = -((rightShoulder.position.x / VIDEO_WIDTH) * 2 - 1);
    meshPosition.x = (x1 + x2) / 2 ;
    //console.log(leftShoulder);
    let y1 = -((leftShoulder.position.y / VIDEO_HEIGHT) * 2 );
    let y2 =-((rightShoulder.position.y / VIDEO_HEIGHT) * 2 );
