<script>
  ...
  let mesh, meshPosition, pivot, eyesPosition, leftEarPosition,rightEarPosition;
  let camera;
  ...
  function animate() {
    if (mesh && pivot && poses) {
      ...
      xCo = ((boundingBox2D.min.x + 1) * VIDEO_WIDTH) / 2;
      yCo = ((1 - boundingBox2D.max.y) * VIDEO_HEIGHT) / 2;
      const wDm =
  ...
  $: if (poses && mesh) {
    ...
    const leftEar = getPart("leftEar", poses[0].pose)[0];
    const rightEar = getPart("rightEar", poses[0].pose)[0];
    leftEarPosition = leftEar.position.x / VIDEO_WIDTH;
    rightEarPosition = rightEar.position.x / VIDEO_WIDTH;
